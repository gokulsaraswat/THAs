$(document).ready(function() {
	...
	if (navigator.userAgent.search("Safari") >= 0 && navigator.userAgent.search("Chrome") < 0) {
	  ...
	  var cards = ['linux', 'ubuntu', 'windows', 'discord', 'sandroid', 'android'];
	  ...
	  $('.flip-card').click(function(){
		  ...
		  if (!gameStarted && !running){//before the game starts, show all cards to the user and flip back
			  ...
			  $(this).toggleClass('flip-card-inner');
		      });
		  ...
		  else if ($(this).find('.back').attr('id') == chosenCards[0] && chosenCards[1] == null && $(this).hasClass('flip-card-inner') && !running) {
		  ...
		  else if ($(this).hasClass('flip-card-inner')) {
			      
		      return;//if the card clicked is already flipped, return
		      
		  }
	      
		  else if (chosenCards[0] == null && chosenCards[1] == null && !$(this).hasClass('flip-card-inner') && !running) {
			  ...
			  setTimeout(function(){//flip back the chosen cards that did not match
				  ...
				  $(this).closest('.flip-card-inner').toggleClass('flip-card-inner');
			      });
